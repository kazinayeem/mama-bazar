...
async function measure(name, fn, runs = 10) {
    // warm up
    await fn();
    await fn();
    const times = [];
...
async function main() {
    ...
    await measure("1. Price ASC (7 Joins - Current)", () => db_1.db.execute(drizzle_orm_1.sql.raw(`
    ...
    await measure("2. Price ASC (Derived Table + 12 Joins)", () => db_1.db.execute(drizzle_orm_1.sql.raw(`
    ...
    await measure("3. Price DESC (7 Joins - Current)", () => db_1.db.execute(drizzle_orm_1.sql.raw(`
      SELECT products.id, products.title, products.price, categories.name AS categoryName 
      FROM products 
      LEFT JOIN categories ON products.category_id = categories.id 
      LEFT JOIN categories AS sc ON products.sub_category_id = sc.id 
    ...
    await measure("4. Price DESC (Derived Table + 12 Joins)", () => db_1.db.execute(drizzle_orm_1.sql.raw(`
      SELECT p.*, categories.name AS categoryName, brands.name AS brandName, brands.logo AS brandLogo
      FROM (
    ...
    `)));
    // 5. Sale=true Current
    await measure("5. Sale=true (Current)", () => db_1.db.execute(drizzle_orm_1.sql.raw(`
    ...
    await measure("6. Sale=true (Derived Table)", () => db_1.db.execute(drizzle_orm_1.sql.raw(`
      SELECT p.*, categories.name AS categoryName, brands.name AS brandName, brands.logo AS brandLogo
      FROM (
    ...
    `)));
    // 7. Reviews Current
    await measure("7. Reviews (Current Left Join)", () => db_1.db.execute(drizzle_orm_1.sql.raw(`
    ...
    await measure("8. Reviews (Two-Step: Reviews first, then batch product info)", async () => {
        const [revs] = await db_1.db.execute(drizzle_orm_1.sql.raw(`
      ...
      LIMIT 12
    `));
        const pids = [...new Set(revs.map((r) => r.product_id).filter(Boolean))];
        if (pids.length > 0) {
            await db_1.db.execute(drizzle_orm_1.sql.raw(`
...
//# sourceMappingURL=benchmark_strategies.js.map
...
const run = async () => {
    try {
        ...
        for (const [column, definition] of productColumns) {
            await db_1.db.execute((0, drizzle_orm_1.sql) `ALTER TABLE products ADD COLUMN IF NOT EXISTS ${drizzle_orm_1.sql.raw(column)} ${drizzle_orm_1.sql.raw(definition)}`);
        }
        ...
        for (const [column, definition] of categoryColumns) {
            await db_1.db.execute((0, drizzle_orm_1.sql) `ALTER TABLE categories ADD COLUMN IF NOT EXISTS ${drizzle_orm_1.sql.raw(column)} ${drizzle_orm_1.sql.raw(definition)}`);
        }
        // ---------- colors ----------
...
run();
//# sourceMappingURL=migrate-catalog.js.map
...
const run = async () => {
    ...
    const extras = config.sections
        ...
        .map((s) => (byId.get(s.id) ? byId.get(s.id) : s));
    ...
    console.log("Homepage config reordered:", config.sections.map((s) => `${s.id}(${s.enabled ? "on" : "off"})`).join(" -> "));
};
run()
    .then(() => process.exit(0))
    .catch((err) => {
...
//# sourceMappingURL=migrate-homepage-layout.js.map
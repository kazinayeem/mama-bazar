...
const run = async () => {
    ...
    `);
        ...
        const getColumnType = async (table, column) => {
            const rows = await db_1.db.execute((0, drizzle_orm_1.sql) `SELECT COLUMN_TYPE FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ${table} AND COLUMN_NAME = ${column}`);
            const r = rows;
      ...
      `);
        ...
        for (const [column, definition] of addressColumns) {
            await db_1.db.execute((0, drizzle_orm_1.sql) `ALTER TABLE user_addresses ADD COLUMN IF NOT EXISTS ${drizzle_orm_1.sql.raw(column)} ${drizzle_orm_1.sql.raw(definition)}`);
        }
...
run();
//# sourceMappingURL=migrate-checkout.js.map
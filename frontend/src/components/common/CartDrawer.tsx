    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          <motion.aside
            animate={{ x: 0 }}
            className="fixed inset-y-0 right-0 z-[200] flex w-full max-w-md flex-col bg-white shadow-lift dark:bg-slate-900"
            exit={{ x: '100%' }}
            initial={{ x: '100%' }}
          ...
          >
            ...
            {items.length === 0 ? (
              <div className="flex flex-1 flex-col items-center justify-center gap-3 p-8 text-center">
                <span className="flex h-20 w-20 items-center justify-center rounded-full bg-slate-100 text-slate-400 dark:bg-slate-800">
                  <ShoppingBag size={32} />
                </span>
              ...
              <>
                <div className="flex-1 overflow-y-auto px-5 py-4">
                  <div className="space-y-4">
                    {items.map((item) => {
                      const price = typeof item.product.price === 'string' ? Number(item.product.price) : item.product.price
                      return (
                        <div className="flex gap-3 rounded-[18px] bg-slate-50 p-3 dark:bg-slate-800/60" key={item.key}>
                          ...
                          <div className="flex min-w-0 flex-1 flex-col">
                            ...
                            <div className="mt-auto flex items-center justify-between pt-2">
                              ...
                              <p className="text-sm font-extrabold text-slate-900 dark:text-white">{formatPrice(price * item.quantity)}</p>
                            </div>
                          </div>
                        </div>
                      )
                    ...
                    <div className="mt-6">
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        {recommendations.map((product) => (
                          <Link
                            className="group flex flex-col overflow-hidden rounded-xl bg-slate-50 transition hover:-translate-y-0.5 hover:shadow-soft dark:bg-slate-800/60"
                            key={product.id}
                            onClick={() => dispatch(closeCart())}
                ...
                <footer className="border-t border-slate-100 p-5 dark:border-slate-800">
                  <div className="space-y-1.5 text-sm">
                    ...
                    <div className="flex justify-between border-t border-dashed border-slate-200 pt-2 text-base font-extrabold text-slate-900 dark:border-slate-700 dark:text-white">
                      <span>Total</span>

...
import type { HomepageSectionConfig, HomepageFlashSaleWindow } from '../../types/homepage'
import type { Product } from '../../types'

// Reusable skeletons with reserved height to guarantee 0 CLS
...
export const ProgressiveCategories = ({
  ...
  const { data: categories = [], isLoading } = useGetCategoriesQuery(undefined, { skip: !inView })
...
export const ProgressiveProductRail = ({
  ...
  const queryParams = useMemo(
    ...
    [label, categorySlug, section.limit],
  )

  const { data, isLoading } = useGetProductsQuery(queryParams, { skip: !inView })
...
export const ProgressiveFlashDeals = ({
  ...
  const { data, isLoading } = useGetProductsQuery({ label: 'flash_sale', limit: section.limit || 12 }, { skip: !inView })
...
export const ProgressiveBrands = ({
  ...
  const { data: brands = [], isLoading } = useGetBrandsQuery(undefined, { skip: !inView })
...
export const ProgressiveCollections = ({
  ...
  const { data: collections = [], isLoading } = useGetCollectionsQuery(undefined, { skip: !inView })
...
export const ProgressiveReviews = ({
  ...
  const { data: reviews = [], isLoading } = useGetReviewsQuery({ limit: section.limit || 8 }, { skip: !inView })

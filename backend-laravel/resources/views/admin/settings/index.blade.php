...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <form action="{{ route('admin.settings.update') }}" method="POST" class="bg-white p-6 sm:p-8 admin-surface space-y-4">
        ...
        <div>
            ...
            <textarea name="contact_address" rows="2" class="w-full text-xs rounded-xl border border-slate-200 p-2.5 focus:border-brand-green-500 focus:outline-none">{{ $settings['contact_address'] ?? 'Dhaka, Bangladesh' }}</textarea>
        </div>
        ...
        <div class="pt-4 border-t border-slate-100 flex justify-end">
            <button type="submit" class="px-6 py-2.5 rounded-xl bg-brand-green-600 hover:bg-brand-green-700 text-white font-bold text-xs shadow-sm transition">
                Save Store Settings
            </button>
        </div>
    </form>

...
<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    <div>
        <h1 class="text-2xl font-bold tracking-tight text-slate-900">{{ $headerTitle }}</h1>
...
<div class="mt-4 flex flex-wrap gap-2">
    @foreach(['overview' => 'Overview', 'monthly' => 'Monthly', 'members' => 'Members', 'categories' => 'Categories', 'profit' => 'Profit'] as $key => $label)
        <a href="{{ route('admin.expenses.reports', ['tab' => $key]) }}"
           class="rounded-full px-4 py-1.5 text-xs font-semibold {{ $tab === $key ? 'bg-brand-green-500 text-white' : 'border border-slate-200 bg-white text-slate-600 hover:bg-slate-50' }}">
            {{ $label }}
        </a>
    @endforeach
</div>
...
@if($tab === 'profit')
    <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div class="rounded-xl border bg-white p-5 shadow-soft"><p class="text-sm text-slate-500">Revenue</p><p class="mt-1 text-2xl font-bold text-brand-green-700">৳{{ number_format($revenue, 0) }}</p></div>
        <div class="rounded-xl border bg-white p-5 shadow-soft"><p class="text-sm text-slate-500">Expenses</p><p class="mt-1 text-2xl font-bold text-red-600">৳{{ number_format($expenseTotal, 0) }}</p></div>
        <div class="rounded-xl border bg-white p-5 shadow-soft"><p class="text-sm text-slate-500">Profit</p><p class="mt-1 text-2xl font-bold {{ $profit >= 0 ? 'text-brand-green-700' : 'text-red-600' }}">৳{{ number_format($profit, 0) }}</p></div>
    </div>
